...
public class Customer {
    ...
    public double getTotalAmount() {
        ...
        for (Rental r : _rentals)
            total += r.getMovie().getRentalAmount(r.getDaysRented());
        return total;
    }
    ...
    public int getTotalFrequentRenterPoints() {
        int pts = 0;
        for (Rental r : _rentals)
            pts += r.getMovie().getFrequentRentalPoints(r.getDaysRented());
        return pts;
    }
    ...
    public String statement() {
        ...
        for (Rental each : _rentals) {
            double amount = each.getMovie().getRentalAmount(each.getDaysRented());
            result.append("\t")
